...
export class EratzIrService {
  ...
  async startGame(roomId: string) {
    ...
    gameState.totalScores = gameState.players.reduce(
      (acc, player) => {
        acc[player] = 0;
        return acc;
      },
  ...
  async startRound(
    roomId: string,
    categories: string[] = ['עיר', 'ארץ', 'חי', 'צומח'],
  ) {
    const gameState = this.gameStates.get(roomId);
    if (!gameState) throw new NotFoundException('לא נמצא משחק לחדר הזה');

    const letter =
      this.hebrewLetters[Math.floor(Math.random() * this.hebrewLetters.length)];
  ...
  private async validateAnswers(
    ...
    if (!validationResult.valid) {
      throw new BadRequestException(
        `תשובות לא תקינות: ${validationResult.errors.join(', ')}`,
      );
    }
