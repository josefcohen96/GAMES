...
export class GameService {
    ...
    /** ✅ Actions for Eratz-Ir */
    private handleEratzIrAction(roomId: string, action: string, payload: any) {
        switch (action) {
            case 'start': // מתחיל משחק
                return this.eratzIrService.startGame(roomId, payload.categories);
            case 'save': // שומר תשובות זמניות
                return this.eratzIrService.saveAnswers(roomId, payload.playerId, payload.answers);
            case 'submitAll': // סיום משחק וחישוב תוצאות
                return this.eratzIrService.submitAll(roomId);
            case 'state': // מצב משחק
